describe('ErrorsSearchCtrl', function() {
  ...
  var noResultsResponse = {"totalResults":0,"itemsPerPage":0,"page":0,"messages":null};

  beforeEach(inject(function($rootScope, $controller, $httpBackend, Globals, EsbMessageService, $filter) {
    rootScope = $rootScope;
    scope = $rootScope.$new();
    http = $httpBackend;
    globals = Globals;
    ...
    fromDateStr = filter('date')(fromDate, globals.dateFormat.service);

    //spyOn(MessageService, 'addFailureMessage').andCallThrough();
    //spyOn(termsService, 'updateTerm').andCallThrough();

    (errorsSearchCtrl) = $controller('ErrorsSearchCtrl', {'$scope': scope});

    // not testing autcompletion stuff - those unit tests belong to angucomplete-keyvalue project
    http.expectGET("api/search/suggest/").respond(200);
    // Simulate a server response
    http.flush();
  }));

  it('makes a GET request with parameters built from search criteria', function() {
    ...
    var url = "api/search/criteria/crit{crit}?fromDate={fromDate}&toDate={toDate}&start=0&results=12&sortField=timestamp&sortAsc=true".
      supplant({crit: ';Key=Value;Blah=Bleh;Foo=bar', fromDate: fromDateStr, toDate: toDateStr});
