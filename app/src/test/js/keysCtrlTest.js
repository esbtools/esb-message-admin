describe('SyncKeysCtrl and SearchKeysCtrl', function() {
    ...
    beforeEach(inject(function($rootScope, $controller, $httpBackend, Globals,
        ...
        http.expectGET("api/key/tree/SearchKeys")
        .respond(searchKeysSuccessResponse);
        http.flush();

    }));

    function loadTest(scope , response) {
        expect(scope.entities || scope.searchKeys).toEqual(response.tree);
    ...
    function crumbTest(scope, response) {
        ...
        expect(scope.crumbs[0]).toEqual(response.tree);
        expect(scope.crumbs[1]).toEqual(response.tree.children[0]);
        expect(scope.crumbs[2]).toEqual(response.tree.children[0].children[0]);
        expect(scope.parent).toEqual(response.tree.children[0].children[0]);
    ...
    function addTest(scope, response, name, type, value) {
        ...
        scope.requestAdd();
        http.expectPOST("api/key/addChild/"+scope.parent.id+"?name="+name+"&type="+type+"&value="+value)
        .respond(response);
        http.flush();
    ...
    it('add tests', function() {
        addTest(syncKeysScope, syncKeysSuccessResponse, "testName", "Entity", "testValue");
        addTest(searchKeysScope, searchKeysSuccessResponse, "SearchKeys", "SearchKey", "testValue");
    });
    ...
    function updateTests(scope, response, name, type, value) {
        ...
        scope.requestUpdate();
        http.expectPUT("api/key/update/"+scope.parent.id+"?name="+name+"&type="+type+"&value="+value)
        .respond(response);
        http.flush();
        expect(scope.parent).toEqual(response.result);
        expect(scope.entities || scope.searchKeys).toEqual(response.tree);
    ...
    it('update tests', function() {
        updateTests(syncKeysScope, syncKeysSuccessResponse, "testName", "Entity", "testValue");
        updateTests(searchKeysScope, searchKeysSuccessResponse, "SearchKey", "SearchKey", "testValue");
    });
