...
esbMessageAdminServices
        .service(
                ...
                [
                        ...
                        function($http, $q, Globals, $log, $filter,
                                messageCenterService) {

                            var self = this;

                            self.search = function(criteria, fromDate, toDate,
                                    start, maxResults, sortField, sortAsc) {
                                ...
                                var _fromDate = $filter('date')(fromDate,
                                        Globals.dateFormat.service);
                                var _toDate = $filter('date')(toDate,
                                        Globals.dateFormat.service);

                                return $http(
                                        {
                                            method : 'GET',
                                            url : "api/search/criteria/crit{critPath}?fromDate={fromDate}&toDate={toDate}&start={start}&results={maxResults}&sortField={sortField}&sortAsc={sortAsc}"
                                                    .supplant({
                                                        critPath : critPath,
                                                        fromDate : _fromDate,
                                                        toDate : _toDate,
                                                ...
                                                function(results) {
                                                    ...
                                                    angular
                                                            .forEach(
                                                                    ...
                                                                    function(
                                                                            esbMessage) {
                                                                        esbMessage.timestamp = new Date(
                                                                                esbMessage.timestamp);
                                                                    });
                            ...
                            // fetch keys and values used for search query
                            // autocompletion
                            ...
                            self.updateKey = function(argId, argName, argType,
                                    argValue) {

                                return $http.put(
                                        "api/key/update/{id}?name={name}&type={type}&value={value}"
                                                .supplant({
                                                    id : argId,
                                                    name : argName,
                                                    type : argType,
                                                ...
                                                function(results) {
                                                    if (results.data.status === "Success") {
                                                        messageCenterService
                                                                .add(
                                                                        'success',
                                                                        'Sync was triggered succesfully!',
                                                                        {
                                                                            timeout : 3000
                                                                        });
                                                    } else {
...
esbMessageAdminServices.service('localStorage', function() {
    return window.localStorage;
});
esbMessageAdminServices.service('errorColumnPrefs', ['localStorage', function(localStorage) {
    var self = this;
