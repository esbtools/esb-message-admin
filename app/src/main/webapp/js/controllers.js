...
esbMessageAdminControllers.controller('ErrorsSearchCtrl', ['$scope', '$rootScope', 'EsbMessageService', '$log', 'Globals',
  function($scope, $rootScope, EsbMessageService, $log, Globals) {

    // initialize autocomplete data
    EsbMessageService.getSuggestions().then(function(response) {
    ...
    var columnDefs = [
      {field: 'sourceSystem', displayName: 'Source', width: 110, cellTemplate: standardCellTemplate},
      {field: 'messageType', displayName: 'Type', width: 110, cellTemplate: standardCellTemplate},
      {field: 'timestamp', displayName: 'Timestamp', width: 130, cellTemplate: dateCellTempate},
      {field: 'occurrenceCount', displayName: '#', width: 10, cellTemplate: standardCellTemplate}
    ];
    ...
    var parseSearchString = function(searchStr) {
        ...
        termsArray.forEach(function(term) {
            ...
            if (keyValue.length == 2) {
                criteria[keyValue[0].trim()]=keyValue[1].replace(/"/g, '').trim();
            }
            else {
                if (term)
                  $log.error("Could not parse: "+term);
            }
        });
        return criteria;
    };
    ...
    var prepareResultsPage = function() {
      ...
      if ($rootScope.searchField_searchStr) {
        ...
        EsbMessageService.search(criteria, $scope.fromDate, $scope.toDate, first, maxResults, sortField, sortAsc).then(function(response){
    ...
    $scope.$watch('messageSelections', function() {
        ...
        //     $scope.messageDetails.errorMessage = "The error message";
        //     $scope.messageDetails.errorDetails = "The Stacktrace";
        //     $scope.messageDetails.payload = "The payload";
        // });
    }, true);
    ...
    $scope.toDate = new Date(2014, 10, 20); // TODO: change this now. Using 11/20 because this is when test data ends.
...
esbMessageAdminControllers.controller('ErrorDetailsCtrl', ['$scope', '$rootScope', 'EsbMessageService',
  function($scope, $rootScope, EsbMessageService) {
    ...
    $rootScope.$watch('selectedMessage', function() {
      if ($scope.selectedMessage) {
        EsbMessageService.getMessage($scope.selectedMessage.id).then(function(result) {
